  ...
  <div class="flex flex-col h-full">
    <div class="h-16 shrink-0 flex items-center gap-2.5 px-5 border-b border-gray-200 dark:border-gray-800">
      <NuxtLink to="/" class="flex items-center gap-2.5">
        <span class="flex items-center justify-center w-8 h-8 rounded-xl bg-gradient-to-br from-fuchsia-400 to-pink-500 text-white shrink-0 shadow-sm">
      ...
      <div v-for="(group, i) in groups" :key="i">
        ...
        <UVerticalNavigation
          ...
          :ui="{
            ...
            active: 'text-fuchsia-600 dark:text-fuchsia-400 bg-fuchsia-50 dark:bg-fuchsia-400/10 before:hidden',
    ...
    <div class="shrink-0 border-t border-gray-200 dark:border-gray-800 px-4 py-3 space-y-3">
      <div class="flex items-center gap-2">
        ...
        <div class="min-w-0">
          ...
          <UBadge v-if="role" :color="isAdmin ? 'pink' : 'gray'" variant="subtle" size="xs">{{ role }}</UBadge>
        </div>
      </div>
...
const props = defineProps<{
  groups: { title?: string; links: { label: string; to: string; icon: string }[] }[]
...
function groupIsActive(i: number) {
  ...
  return group?.links.some(link => route.path === link.to || route.path.startsWith(`${link.to}/`)) ?? false
}
