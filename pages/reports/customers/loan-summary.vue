  ...
  <div>
    ...
    <PageHeader
      ...
      :crumbs="[
        ...
        { label: t('accounting.customerReports.loanSummary.title') }
      ]"
    />
      ...
      <UCard>
        <template #header>
          <span class="font-semibold">{{
            t('accounting.customerReports.loanSummary.statusBreakdownHeader')
          }}</span>
        </template>
        <DataTable
...
const {
  ...
  pending,
  error: fetchError
} = await useAsyncData('customer-loan-summary-loans', () =>
...
const columns = computed<ColumnDef<StatusRow>[]>(() => [
  {
    key: 'status',
    label: t('accounting.customerReports.loanSummary.columns.status'),
    type: 'status'
  },
  { key: 'loanCount', label: t('accounting.customerReports.loanSummary.columns.loanCount') },
  {
    key: 'totalPrincipal',
    label: t('accounting.customerReports.loanSummary.columns.totalPrincipal'),
    type: 'currency'
  }
])
</script>
