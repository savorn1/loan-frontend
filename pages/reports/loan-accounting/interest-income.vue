<template>
  <div>
    ...
    <UCard class="mb-6">
      <UFormGroup :label="t('accounting.loanAccountingReports.interestIncome.financialPeriod')" class="max-w-xs">
        <USelectMenu
          v-model="financialPeriodId"
          :options="periodOptions"
          option-attribute="label"
          value-attribute="value"
          :placeholder="t('accounting.loanAccountingReports.interestIncome.financialPeriodPlaceholder')"
        />
      </UFormGroup>
    </UCard>
    ...
    <UCard v-if="ledger" class="mb-6">
      <dl class="grid grid-cols-2 sm:grid-cols-4 gap-y-3 text-sm">
        <dt class="text-gray-500">{{ t('accounting.loanAccountingReports.interestIncome.openingBalance') }}</dt>
        <dd class="font-semibold">{{ formatCurrency(ledger.openingBalance) }}</dd>
        <dt class="text-gray-500">{{ t('accounting.loanAccountingReports.interestIncome.periodDebit') }}</dt>
        <dd>{{ formatCurrency(ledger.periodDebitTotal) }}</dd>
        <dt class="text-gray-500">{{ t('accounting.loanAccountingReports.interestIncome.periodCredit') }}</dt>
        <dd>{{ formatCurrency(ledger.periodCreditTotal) }}</dd>
        <dt class="text-gray-500">{{ t('accounting.loanAccountingReports.interestIncome.closingBalance') }}</dt>
...
const columns = computed<ColumnDef<LedgerLineResponse>[]>(() => [
  { key: 'transactionDate', label: t('accounting.loanAccountingReports.interestIncome.columns.date'), type: 'date' },
  { key: 'entryNo', label: t('accounting.loanAccountingReports.interestIncome.columns.entryNo') },
  { key: 'description', label: t('accounting.loanAccountingReports.interestIncome.columns.description') },
  {
    key: 'entrySide',
  ...
  { key: 'amount', label: t('accounting.loanAccountingReports.interestIncome.columns.amount'), type: 'currency' },
  { key: 'runningBalance', label: t('accounting.loanAccountingReports.interestIncome.columns.balance'), type: 'currency' }
])
</script>
