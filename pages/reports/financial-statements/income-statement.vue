<template>
  <div>
    ...
    <UCard class="mb-6">
      <UFormGroup :label="t('accounting.financialStatements.incomeStatement.financialPeriod')" class="max-w-xs">
        <USelectMenu
          v-model="financialPeriodId"
          :options="periodOptions"
          option-attribute="label"
          value-attribute="value"
          :placeholder="t('accounting.financialStatements.incomeStatement.financialPeriodPlaceholder')"
        />
      </UFormGroup>
    </UCard>
    ...
    <template v-else-if="!pending && !hasAnyActivity">
      <UCard>
        <EmptyState
          ...
          :description="t('accounting.financialStatements.incomeStatement.emptyDescriptionNoActivity')"
        />
      </UCard>
    </template>
    <template v-else>
      <UCard class="mb-6">
        <template #header>
          <span class="font-semibold">{{ t('accounting.financialStatements.incomeStatement.income') }}</span>
        </template>
        ...
        <div class="flex justify-end pt-4 px-2 text-sm font-semibold border-t border-gray-200 dark:border-gray-800 mt-2">
          <span>{{ t('accounting.financialStatements.incomeStatement.totalIncome') }}: {{ formatCurrency(totalIncome) }}</span>
        </div>
      </UCard>

      <UCard class="mb-6">
        <template #header>
          <span class="font-semibold">{{ t('accounting.financialStatements.incomeStatement.expenses') }}</span>
        </template>
        <DataTable :rows="expenseLines" :columns="columns" :loading="pending" :exportable="false" />
        <div class="flex justify-end pt-4 px-2 text-sm font-semibold border-t border-gray-200 dark:border-gray-800 mt-2">
          <span>{{ t('accounting.financialStatements.incomeStatement.totalExpenses') }}: {{ formatCurrency(totalExpenses) }}</span>
        </div>
      </UCard>
      ...
      <UCard>
        <div class="flex justify-end text-base font-bold">
          <span :class="netIncome >= 0 ? 'text-teal-600 dark:text-teal-400' : 'text-red-500'">
            {{ netIncome >= 0 ? t('accounting.financialStatements.incomeStatement.netIncome') : t('accounting.financialStatements.incomeStatement.netLoss') }}:
...
const columns = computed<ColumnDef<StatementLine>[]>(() => [
  { key: 'accountNo', label: t('accounting.financialStatements.incomeStatement.columns.accountNo') },
  { key: 'accountName', label: t('accounting.financialStatements.incomeStatement.columns.accountName') },
  { key: 'amount', label: t('accounting.financialStatements.incomeStatement.columns.amount'), type: 'currency' }
])
</script>
