  ...
  <div>
    ...
    <PageHeader
      ...
      :crumbs="[
        ...
        { label: t('accounting.penaltyReports.charges.title') }
      ]"
    />
    ...
    <UCard v-if="total > 0" class="mb-6">
      <div class="flex justify-end text-sm font-semibold">
        <span
          >{{ t('accounting.penaltyReports.charges.totalCharged') }}:
          {{ formatCurrency(totalCharged) }}</span
        >
      </div>
    </UCard>
...
const {
  ...
  pending,
  error: fetchError
} = await useAsyncData('penalty-charges', () =>
...
const chargedInRange = computed(() => {
  ...
  return penalties.value.filter(
    (p) => p.appliedDate >= dateFrom.value && p.appliedDate <= dateTo.value
  )
})
...
const columns = computed<ColumnDef<LoanPenaltyResponse>[]>(() => [
  ...
  {
    key: 'amount',
    label: t('accounting.penaltyReports.charges.columns.amount'),
    type: 'currency',
    sortable: true
  },
  ...
  {
    key: 'appliedDate',
    label: t('accounting.penaltyReports.charges.columns.appliedDate'),
    type: 'date',
    sortable: true
  },
  {
    key: 'status',
    label: t('accounting.penaltyReports.charges.columns.status'),
    type: 'status',
    sortable: true
  }
])
