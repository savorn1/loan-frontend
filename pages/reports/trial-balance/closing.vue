<template>
  <div>
    ...
    <UCard class="mb-6">
      <UFormGroup :label="t('accounting.trialBalanceReports.closingTrialBalance.financialPeriod')" class="max-w-xs">
        <USelectMenu
          v-model="financialPeriodId"
          :options="periodOptions"
          option-attribute="label"
          value-attribute="value"
          :placeholder="t('accounting.trialBalanceReports.closingTrialBalance.financialPeriodPlaceholder')"
        />
      </UFormGroup>
    </UCard>
    ...
    <UCard v-if="latestSnapshot" class="mb-6">
      <dl class="grid grid-cols-2 sm:grid-cols-4 gap-y-3 text-sm">
        <dt class="text-gray-500">{{ t('accounting.trialBalanceReports.closingTrialBalance.generatedAt') }}</dt>
        <dd class="font-semibold">{{ formatDateTime(latestSnapshot.generatedAt) }}</dd>
        <dt class="text-gray-500">{{ t('accounting.trialBalanceReports.closingTrialBalance.generatedBy') }}</dt>
    ...
    <UCard>
      ...
      >
        <span>{{ t('accounting.trialBalanceReports.closingTrialBalance.totalDebit') }}: {{ formatCurrency(latestSnapshot.totalDebit) }}</span>
        <span>{{ t('accounting.trialBalanceReports.closingTrialBalance.totalCredit') }}: {{ formatCurrency(latestSnapshot.totalCredit) }}</span>
...
const columns = computed<ColumnDef<TrialBalanceRowResponse>[]>(() => [
  { key: 'accountNo', label: t('accounting.trialBalanceReports.closingTrialBalance.columns.accountNo'), sortable: true },
  { key: 'accountName', label: t('accounting.trialBalanceReports.closingTrialBalance.columns.accountName'), sortable: true },
  { key: 'totalDebit', label: t('accounting.trialBalanceReports.closingTrialBalance.columns.debit'), type: 'currency', sortable: true },
  { key: 'totalCredit', label: t('accounting.trialBalanceReports.closingTrialBalance.columns.credit'), type: 'currency', sortable: true },
  { key: 'balance', label: t('accounting.trialBalanceReports.closingTrialBalance.columns.balance'), type: 'currency', sortable: true }
])
