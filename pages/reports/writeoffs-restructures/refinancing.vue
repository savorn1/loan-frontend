...
const refinancesInRange = computed(() => {
  ...
  return refinances.value.filter((r) => r.effectiveDate >= dateFrom.value && r.effectiveDate <= dateTo.value)
})
...
const columns = computed<ColumnDef<LoanRefinanceResponse>[]>(() => [
  { key: 'loanId', label: t('accounting.writeoffRestructureReports.refinancing.columns.loanId'), sortable: true },
  {
    key: 'newLoanId',
