<template>
  <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <UFormGroup :label="t('accounting.branchAccountingReports.transactionSummary.branch')">
          <USelectMenu
            ...
            :placeholder="t('accounting.branchAccountingReports.transactionSummary.branchPlaceholder')"
          />
        </UFormGroup>
    ...
    <UCard>
      ...
      <DataTable :rows="rows" :columns="columns" :loading="pending">
        <template #empty-state>
          <EmptyState
            ...
            :description="
              branchId && hasFullRange
                ? t('accounting.branchAccountingReports.transactionSummary.emptyDescriptionNoActivity')
      ...
      >
        <span>{{ t('accounting.branchAccountingReports.transactionSummary.totalDebit') }}: {{ formatCurrency(totalDebit) }}</span>
        <span>{{ t('accounting.branchAccountingReports.transactionSummary.totalCredit') }}: {{ formatCurrency(totalCredit) }}</span>
      </div>
    </UCard>
  </div>
</template>
...
const { data: branches } = await useAsyncData('branch-txn-summary-branches', () => api<BranchResponse[]>('/branches'))
...
const columns = computed<ColumnDef<SummaryRow>[]>(() => [
  { key: 'accountNo', label: t('accounting.branchAccountingReports.transactionSummary.columns.accountNo'), sortable: true },
  { key: 'accountName', label: t('accounting.branchAccountingReports.transactionSummary.columns.accountName'), sortable: true },
  { key: 'debit', label: t('accounting.branchAccountingReports.transactionSummary.columns.debit'), type: 'currency', sortable: true },
  { key: 'credit', label: t('accounting.branchAccountingReports.transactionSummary.columns.credit'), type: 'currency', sortable: true },
  { key: 'net', label: t('accounting.branchAccountingReports.transactionSummary.columns.net'), type: 'currency', sortable: true }
])
</script>
