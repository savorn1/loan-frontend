  ...
  <div>
    ...
    <UCard v-if="hasFullRange" class="mb-6">
      <dl class="grid grid-cols-3 gap-y-3 text-sm">
        <dt class="text-gray-500">{{ t('accounting.repaymentReports.collection.totalCollected') }}</dt>
        <dd class="font-semibold col-span-2 text-right">{{ formatCurrency(totalCollected) }}</dd>
        <dt class="text-gray-500">{{ t('accounting.repaymentReports.collection.totalPrincipal') }}</dt>
        <dd class="font-semibold col-span-2 text-right">{{ formatCurrency(totalPrincipal) }}</dd>
        <dt class="text-gray-500">{{ t('accounting.repaymentReports.collection.totalInterest') }}</dt>
...
const {
  ...
  pending,
  error: fetchError
} = await useAsyncData('repayments-collection', () => api<PageResponse<PaymentResponse>>('/payments', { query: { size: 1000 } }))
...
const totalPrincipal = computed(() => collected.value.reduce((sum, p) => sum + (p.principalComponent ?? 0), 0))
const totalInterest = computed(() => collected.value.reduce((sum, p) => sum + (p.interestComponent ?? 0), 0))
...
const columns = computed<ColumnDef<PaymentResponse>[]>(() => [
  { key: 'loanId', label: t('accounting.repaymentReports.collection.columns.loanId'), sortable: true },
  { key: 'installmentNumber', label: t('accounting.repaymentReports.collection.columns.installmentNumber'), sortable: true },
  { key: 'paidAt', label: t('accounting.repaymentReports.collection.columns.paidAt'), type: 'date', sortable: true },
  { key: 'amount', label: t('accounting.repaymentReports.collection.columns.amount'), type: 'currency', sortable: true },
  { key: 'principalComponent', label: t('accounting.repaymentReports.collection.columns.principalComponent'), type: 'currency' },
  { key: 'interestComponent', label: t('accounting.repaymentReports.collection.columns.interestComponent'), type: 'currency' }
])
</script>
