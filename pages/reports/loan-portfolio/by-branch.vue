...
const {
  ...
  pending,
  error: fetchError
} = await useAsyncData('loan-portfolio-by-branch', () => api<PageResponse<LoanResponse>>('/loans', { query: { size: 1000 } }))
const loans = computed(() => loansRaw.value?.content ?? [])

const { data: branches } = await useAsyncData('loan-portfolio-by-branch-branches', () => api<BranchResponse[]>('/branches'))
...
const rows = computed<BranchLoanRow[]>(() => {
  ...
  for (const loan of loans.value) {
    const branch = loan.branchId != null ? (branchNameById.get(loan.branchId) ?? String(loan.branchId)) : '—'
    const existing = byBranch.get(branch) ?? { branch, loanCount: 0, totalPrincipal: 0, totalOutstanding: 0 }
...
const columns = computed<ColumnDef<BranchLoanRow>[]>(() => [
  { key: 'branch', label: t('accounting.loanPortfolioReports.byBranch.columns.branch'), sortable: true },
  { key: 'loanCount', label: t('accounting.loanPortfolioReports.byBranch.columns.loanCount'), sortable: true },
  { key: 'totalPrincipal', label: t('accounting.loanPortfolioReports.byBranch.columns.totalPrincipal'), type: 'currency', sortable: true },
  {
