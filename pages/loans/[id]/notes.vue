<template>
  <UCard>
    ...
    <UForm :state="form" class="flex items-start gap-2 mb-6" @submit="onCreate">
      <UTextarea v-model="form.note" :rows="2" placeholder="Add an internal note..." class="flex-1" required />
      <UButton type="submit" :loading="creating" :disabled="!form.note.trim()">Add</UButton>
    </UForm>

    <ol v-if="(notes ?? []).length" class="space-y-4">
      <li v-for="n in sortedNotes" :key="n.id" class="border-b border-gray-100 dark:border-gray-800 pb-3 last:border-0 last:pb-0">
        <p class="text-sm whitespace-pre-wrap">{{ n.note }}</p>
        <p class="text-xs text-gray-500 mt-1">{{ n.authorName }} · {{ formatDateTime(n.createdAt) }}</p>
      </li>
    </ol>
    <EmptyState v-else icon="i-heroicons-chat-bubble-left-right" title="No notes yet" description="Internal notes and comments about this loan will appear here." />
  </UCard>
</template>
