      ...
      <UForm :state="documentForm" class="space-y-2 mb-6" @submit="onAddDocument">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <UInput
            ...
            :placeholder="t('groupLoanApplications.detail.fileNamePlaceholder')"
          />
          <UInput
            v-model="documentForm.fileUrl"
            :placeholder="t('groupLoanApplications.detail.fileUrlPlaceholder')"
          />
        </div>
        <div class="flex justify-end">
          <UButton type="submit" size="xs" :loading="addingDocument" :disabled="!canAddDocument">{{
            t('groupLoanApplications.detail.addDocument')
          }}</UButton>
        </div>
      </UForm>
...
import type {
  ...
  GroupLoanApplicationDocumentRequest,
...
async function onAddDocument() {
  ...
  try {
    const payload: GroupLoanApplicationDocumentRequest = { ...documentForm }
    await api(`/group-loan-applications/${applicationId}/documents`, {
      method: 'POST',
      body: payload
    })
